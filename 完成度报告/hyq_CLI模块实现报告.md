...
### 1. 方向键导航实现情况（基于 termios）

**问题描述（旧版本）**:
- 早期版本的 `read_input()` 使用 `fgets()` 读取输入，无法检测方向键等特殊按键
- `get_history_command()` 函数当时已实现但未被调用

**当前结论（最新代码）**:
- 已改为使用 `termios + read()` 实现原始输入模式，可检测方向键 / Delete / 退格 / Ctrl+D 等
- 在处理 ESC 转义序列时，会调用 `get_history_command(cli, -1/1)` 实现历史导航
