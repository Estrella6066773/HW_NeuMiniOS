...
**为什么删除进程用链表会更方便？（用通俗易懂的方式解释）**

#### 数组删除的问题（就像一排固定位置的座位）

想象一下，数组就像一排**固定位置的座位**（座位 0、1、2、3、4），每个座位只能坐一个人：

- **问题 1：删除后留下“空座位”**
  - 如果座位 2 的人走了，你只是把座位 2 标记为“空”，但座位还在那里。
  - 当你想要“列出所有坐着的人”时，你必须**一个一个座位检查**：座位 0 有人吗？座位 1 有人吗？座位 2 是空的，跳过；座位 3 有人吗？...
  - 这样每次遍历都要判断每个座位是不是空的，效率低，而且数组中会留下很多“洞”。

- **问题 2：如果要保持连续，需要“搬家”**
  - 如果你不想有空座位，想把座位 3、4 的人往前移（坐到座位 2、3），那就要：
    1. 把座位 3 的人搬到座位 2
    2. 把座位 4 的人搬到座位 3
    3. 更新所有相关的记录
  - 这个过程就像**搬家**，如果数组有 5 个元素，删除中间一个，后面所有元素都要往前移，代码复杂，还容易出错。

#### 链表删除的优势（就像手拉手的一队人）

链表就像**手拉手的一队人**，每个人只记住“我拉着谁的手”：

- **删除只需要“换手”**
  - 假设队伍是：A → B → C → D（A 拉着 B，B 拉着 C，C 拉着 D）
  - 如果要删除 B，只需要：
    1. 让 A 不再拉 B 的手，改为拉 C 的手（`A->next = C`）
    2. 把 B 从队伍里放走（`free(B)`）
  - 就这么简单！**不需要移动 C 和 D**，他们还在原来的位置，只是现在 A 直接拉着 C 了。

- **查找和删除一次完成**
  - 在 `stop_process` 里，`find_process(pid, &prev)` 函数会：
    1. 从队伍头开始，一个人一个人地找（就像你沿着手拉手的队伍走）
    2. 找到目标进程的同时，也记住了“它前面是谁”（前驱 `prev`）
    3. 然后只需要让前一个人改拉目标的下一个人，就完成了删除

#### 具体代码对比

**数组删除（需要移动元素）：**
```c
// 假设要删除 process_table[2]
// 需要把后面的元素都往前移
for (int i = 2; i < process_count - 1; i++) {
    process_table[i] = process_table[i + 1];  // 搬家！
}
process_count--;
```
...
// 只需要让前一个节点跳过目标，直接连到下一个
if (prev) prev->next = target->next;  // 换手！
else process_list = target->next;     // 如果是头结点，更新头指针
free(target);  // 释放内存
process_count--;
```

**总结：**
- 数组删除：要么留下空座位（查找麻烦），要么需要“搬家”（代码复杂）
- 链表删除：只需要“换手”（改一个指针），不需要移动其他节点，代码简单、效率高

---

## 8. `process.c` 中所有函数的通俗解释

### 8.1 `find_process` —— 在链表中查找进程（辅助函数）

**作用：** 就像在一队手拉手的人里找某个人，同时记住“它前面是谁”。

**通俗解释：**
- 从队伍头（`process_list`）开始，一个人一个人地检查
- 如果找到目标 PID，就返回这个节点，同时通过 `out_prev` 参数告诉调用者“它前面是谁”（前驱节点）
- 如果找不到，返回 `NULL`

**为什么需要前驱节点？**  
因为删除节点时，需要让“前一个人”改拉“后一个人”的手，所以必须知道前驱是谁。

### 8.2 `init_process_table` —— 初始化进程表

**作用：** 系统启动时，把进程链表清空，确保没有残留的进程记录。

**通俗解释：**
- 就像新学期开始，把所有座位清空，计数器归零
- 遍历整个链表，把每个节点都释放掉（`free`）
- 把链表头设为 `NULL`，进程计数和 PID 计数器都重置

### 8.3 `create_process` —— 创建新进程（`run` 命令的核心）

**作用：** 从磁盘镜像里提取可执行文件，创建子进程运行它，并登记到进程链表。

**通俗解释（分步骤）：**
1. **检查容量**：看看进程表是不是已经满了（最多 5 个）
2. **读取文件**：从磁盘镜像里把可执行文件读出来
3. **写入临时文件**：把文件写到 `/tmp` 目录，并设置成可执行
4. **创建子进程**：
   - 用 `fork()` 复制当前进程，得到一个子进程
   - 子进程用 `execl()` 执行那个临时文件（替换成新程序）
   - 父进程继续往下走
5. **登记到链表**：
   - 用 `malloc` 分配一个新的 `Process` 节点
   - 填写进程信息（PID、系统 PID、名字、状态）
   - 把这个节点**追加到链表尾部**（就像新来的人排在队伍最后）

### 8.4 `stop_process` —— 停止进程（`stop` 命令的核心）

**作用：** 找到指定 PID 的进程，终止它，并从链表中删除记录。

**通俗解释（分步骤）：**
1. **查找进程**：用 `find_process` 在链表里找到目标进程和它的前驱
2. **检查进程是否还存在**：用 `kill(system_pid, 0)` 探测 Linux 系统里的进程是否还在运行
   - 如果已经不存在：直接从链表里摘掉这个节点（避免“幽灵记录”）
   - 如果还在运行：继续下一步
3. **终止进程**：
   - 用 `kill(SIGTERM)` 发送终止信号
   - 用 `waitpid` 等待子进程真正退出（避免僵尸进程）
4. **从链表删除**：
   - 让前驱节点跳过目标节点，直接连到下一个（`prev->next = target->next`）
   - 如果是头结点，更新链表头（`process_list = target->next`）
   - 释放节点内存（`free(target)`）

### 8.5 `list_processes` —— 列出所有进程（`plist` 命令的核心）

**作用：** 遍历进程链表，把所有“运行中”的进程打印成表格。

**通俗解释：**
- 从链表头开始，一个人一个人地检查
- 只打印 `status == 1` 的进程（正在运行的）
- 统计总数，如果没有任何进程就提示用户

### 8.6 `cleanup_process_table` —— 清理进程表（系统退出时调用）

**作用：** 系统退出前，把所有还在运行的子进程都终止掉，并释放所有链表节点。

**通俗解释：**
- 遍历整个链表
- 对每个还在运行的进程（`status == 1`），发送 `SIGKILL` 强制终止，并用 `waitpid` 等待它退出
- 释放所有节点的内存
- 把链表头设为 `NULL`，计数器归零

**为什么需要这个函数？**  
如果不清理，系统退出后，那些子进程可能还在后台运行，造成资源浪费。
